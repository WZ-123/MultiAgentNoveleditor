# Codex 登录态 Luna medium 十万字流程验证

已形成30章验证稿，净正文 **104,517汉字**。前期DeepSeek稿件保留，后续真实生成和编辑均使用Codex登录态的gpt-5.6-luna、medium思考强度；逐章来源与hash见manuscript-provenance.json。正文、设定、角色卡、推理和测试日志分别计数，不以API token数代替小说字数。
...
| --- | ---: | ---: | ---: | ---: | ---: |
| 011-013 | 311.1 | 10,163 | 10 | 2 | 2,959,348 |
| 014-016 | 410.9 | 10,009 | 11 | 4 | 4,149,133 |
| 017-020 | 386.9 | 14,031 | 3 | 2 | 497,914 |
| 021-023 | 262.4 | 10,645 | 2 | 0 | 322,220 |
| 024-026 | 257.8 | 10,573 | 2 | 0 | 397,123 |
| 027-030-finale | 539.0 | 14,009 | 8 | 3 | 2,371,086 |
...
本阶段17个模型回合累计 **53.4分钟**。从首个测试回合启动至最后记录约 **71.2分钟**，两者差额包括诊断、重启和外层分析等待。已知累计usage 18,529,249 tokens，其中输入18,377,034（缓存17,628,416）、输出148,973（推理16,854）。这些不是新增文本量、准确账单或模型纯生成耗时。
...
- 净正文计数、逐章来源hash、全文汇编。长度至少60汉字的完全相同段落组：0；这只是机械重复检查，不等于文学质量验收。
