...
**问题：** 驱动路径（`claude --print` 无头模式）下，WebSearch/WebFetch 被 Claude Code 拒绝，提示 `Claude requested permissions to use WebSearch/WebFetch, but you haven't granted it yet.`

**错误的修复：** 从 `DEFAULT_BUILTIN_TOOLS` 中移除 WebSearch/WebFetch。AI 失去网页搜索能力。

**正确的修复：** 在 tmpdir 中写入 `.claude/settings.json`，预授权这些工具：
...
Claude Code 启动时读取项目级 `.claude/settings.json`，`permissions.allow` 中的工具自动获得授权。
...
| 阻塞类型 | 解决方案 |
|----------|----------|
| Claude Code 工具权限 | `.claude/settings.json` → `permissions.allow` |
