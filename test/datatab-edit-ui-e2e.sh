...
TEST_USERDATA="$ROOT/tmp-test-datatab-ui-userdata"
rm -rf "$TEST_USERDATA"
...
MANA_USER_DATA_ROOT="$TEST_USERDATA" \
  ...
  "$ELECTRON_BIN" . --no-sandbox --test-datatab-edit-ui 2>&1 | tee /tmp/mana-datatab-ui.log
...
done < /tmp/mana-datatab-ui.log

rm -rf "$TEST_USERDATA"
rm -f /tmp/mana-datatab-ui.log
exit $E2E_EXIT