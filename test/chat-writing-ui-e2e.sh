...
TEST_USERDATA="$ROOT/tmp-test-chat-writing-ui-userdata"
rm -rf "$TEST_USERDATA"
...
MANA_USE_STDIO_MCP=0 \
  ...
  "$ELECTRON_BIN" . --no-sandbox --test-chat-writing-ui-regression 2>&1 | tee /tmp/mana-chat-writing-ui-e2e.log
...
done < /tmp/mana-chat-writing-ui-e2e.log
rm -rf "$TEST_USERDATA"
rm -f /tmp/mana-chat-writing-ui-e2e.log
exit $E2E_EXIT