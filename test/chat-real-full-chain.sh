...
TEST_USERDATA="$ROOT/tmp-test-real-full-chain-userdata"
rm -rf "$TEST_USERDATA"
...
MANA_USER_DATA_ROOT="$TEST_USERDATA" \
  ...
  "$ELECTRON_BIN" . --no-sandbox --test-real-full-chain 2>&1 | tee /tmp/mana-real-full-chain.log
...
done < /tmp/mana-real-full-chain.log

rm -rf "$TEST_USERDATA"
rm -f /tmp/mana-real-full-chain.log
exit $E2E_EXIT