---
name: search-routing
description: 联网搜索路由和策略。告知 Codex 在系统驱动模式下如何进行角色联网搜索补全，包括文化圈检测、来源优先级、查询构建、提取和合并逻辑。
...
> ⚠️ 此 skill 的**单一真相源**在 `knowledge-base/search-routing.md`。本文件是其面向 Codex 驱动的转述。
> 修改搜索逻辑时，须同步更新：
...
### 来源选择
- `east-asian-cn`: **Biligame Wiki 直链优先**（六游子 wiki）→ **Bangumi**（`chii.in` / `api.bgm.tv`）→ 萌娘百科 → Bing → Wikipedia（`networkFetch` 代理）→ DuckDuckGo
- `east-asian-jp`: **Bangumi** → 萌娘百科 → Wikipedia → Bing → DuckDuckGo
...
- Bangumi：优先 `角色名 作品名`，回退 `角色名`；搜索优先走官方角色 API，再用关联作品做消歧
- Wikipedia/Fandom/Bing（western-en）: 使用 `nativeCharName` + `nativeWorkName`
- Fandom 子站发现用 `workSynonyms.js` 的 `WORK_SYNONYM_GROUPS`（6 部欧美验收集中英别名）

Biligame 映射：`碧蓝航线`→`blhx`，`原神`→`ys`，`星穹铁道`→`sr`，`绝区零`→`zzz`，`鸣潮`→`wutheringwaves`。API 返 HTML 时用 `biligameWiki.js` 直链；别名见 `bwiki-title-aliases.json`。外网请求走 `HTTPS_PROXY`（`networkFetch.js`）；若浏览器内能访问 Bangumi 但 shell 超时，可用 `scripts/with-electron-proxy.sh` 继承 Electron 实际代理。

欧美 smoke 实测：基线 8/36 → v3 通用层 34/36（见 `western-enrichment-benchmark.md`）。Wikipedia 本环境常超时；萌娘无独立角色页时需 `originalName` 或作品页语境推断。
...
- 若首选信源字段不够满 → 再查第二信源交叉补充，只补缺失字段，不覆盖已锚定字段
